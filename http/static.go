...
func handleWithStaticData(w http.ResponseWriter, _ *http.Request, d *data, file, contentType string) (int, error) {
	w.Header().Set("Content-Type", contentType)
	...
	file = "/frontend/dist/" + file
	fileref, err := pkger.Open(file)
	if err != nil {
		if err == os.ErrNotExist {
			return http.StatusNotFound, err
		}
	...
	buf, err := ioutil.ReadAll(fileref)
	if err != nil {
		return http.StatusInternalServerError, err
	}
...
func getStaticHandlers(store *storage.Storage, server *settings.Server) (index, static http.Handler) {
	handler := http.FileServer(pkger.Dir("/frontend/dist"))
	...
	index = handle(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
		...
		w.Header().Set("x-xss-protection", "1; mode=block")
		return handleWithStaticData(w, r, d, "index.html", "text/html; charset=utf-8")
	}, "", store, server)
	...
	static = handle(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
		...
		return handleWithStaticData(w, r, d, r.URL.Path, "application/javascript; charset=utf-8")
	}, "/static/", store, server)
